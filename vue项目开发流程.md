...
## 从状态中取出goods，动态渲染页面
  ...
  3、左侧列表和右侧商品的上下滑动效果，要用一个库
    ...
    注：左侧和右侧是分开的，就是要有两个实例分别负责，scroll实例与swiper实例一样，都是要等到界面更新后才创建实例。但我们下载的版本是1.0中最新的，内部对页面是否更新有判断，所以我们写的时候直接在组件挂载之后创建即可
    但假设内部是无法判断，要我们自己实现效果的话，我们可以跟swiper的解决方法一样，swiper的解决方法：
      ① watch+$nextTick  
      ② callback+$nextTick 
      ③ 在await 请求后 再创建
    在此处，我们只能使用watch+$nextTick，因为我们这个组件是二级路由组件，而请求是在父组件挂载时就发了，无法传递callback和用await

  4、右侧滑到某一部分，左侧会跟着定位
    为当前的分类设计一个currentIndex,当index与currentIndex相等时，会
      1)、data数据
        右侧每个li的原始top值的数组：tops --- 原始的top值数组，不变
        右侧列表滑动的Y轴坐标：scrollY --- 在滑动时不断改变,通过scroll实例的on绑定‘scroll’事件获取到
        data(){
          return {
      ...
      问题1：刷新后列表丢失
          ...
          "/public/reset.css" 表示相对于根路径，根路径就是请求的路径

      问题2：滑动右侧列表时，使左侧的对应菜单始终可见？
          解决思路：当当前分类项下标（currentIndex）发生改变时，让左侧列表滑动到对应下标的li元素处
            将每一次计算出来的currentIndex保存到状态中，新计算的覆盖之前计算的。并返回
          如何判断下标发生了改变？
            在每次计算下标时，判断状态存的下标是否与currentIndex一致，若是不一致，就是改变了，
            若是改变了，根据计算出的currentIndex得到左侧对应的li元素，再给左侧列表实例对象添加事件scrollToElement，让左侧的滚动到li元素的位置处
  ...
  5、点击左侧某个分类，右侧列表会滑到对应位置
    给每个食物列表绑定点击事件，但注意，scroll会阻止dom的事件行为，若是要点击事件，则要传配置对象：
      ...
      clickMenu(index){
        ...
        this.scrollY = top  // 点击后立马根据top进而计算出currentIndex的值，则左边的图标会立马被选中，否则会有延迟现象
 ...
 # 17 + 组件 Cart
  ...
  问题2：防止点击过快
    解决：用节流 --- lodash的throttle
      ...
      使用：将实际要节流的操作包裹---throttle( function(){ 需要节流的操作 })
...
根据当前state的数据的计算属性写在getters里，写在getters里的计算属性，可以供多个组件使用，并且根据依赖数据改变而重新计算，计算属性一定要有返回值
...
6、若是控制购物车详情显示与隐藏的是v-if，会造成第二次就无法滑动
  原因：第一次初始化时，系统解析了html
  解决：用v-show控制
  关联：在很多时候v-if和v-show可以一起用，但有特定时候只能用v-if，有特定的时候只能用v-show
    v-if：在初始化页面的时候，如果用到了多层数据，会报错，就可以用v-if去控制，因为v-if不会初始化解析
    v-show：只是控制元素的样式显示和隐藏


清空购物车：绑定点击，提示用户是否真的清空，点确定清空---将state中的cartShops数组清空--但是要遍历将food的count都变为0，再清空数组，因为在食物选购页面的显示数量是根据food的count显示的
...
12.19 任务  ： Cart组件的节流、购物车组件
  问题：节流结束时也会触发一次


需总结：防抖和节流 
  防抖和节流在连续高频操作下，连续操作时在一个阈值内连续触发
  防抖
    防抖是防止高频操作，一个连续操作中的处理，只触发一次，从而实现防抖动。
    实现：内部是用定时器实现，在阈值内连续操作时，每次操作都会开启定时器，上一次的操作的定时器被取消，总是执行一个阈值内的最后一次操作
    适用场景：防止用户的多次click提交 
    
  节流
    节流是一个连续操作中的处理，按照阀值时间间隔进行触发，从而实现节流。就是一个阈值之内只触发一次
    实现：内部是用定时器和标识来实现。只有在标识为真时，才能触发定时器，定时器中就是真正要做的操作。执行了定时器，才可以再次使标识为真，循环，形成有规律的触发
    适用场景：滚动条设置定位时的频繁位置计算、拖拽的频繁位置计算、滚轮滚动时无限触发scroll事件等